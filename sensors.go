...
func (sensors *Sensors) processSensors() error {
        ...
        for bank := 0; bank<3; bank++ {
                if (sensors.raw[2+bank*16]>thd  || sensors.raw[4+bank*16]>thd) { sensors.net[0+bank*8]=down }
                if (sensors.raw[11+bank*16]>thd || sensors.raw[13+bank*16]>thd){ sensors.net[1+bank*8]=down }
                if (sensors.raw[3+bank*16]>thd  || sensors.raw[7+bank*16]>thd) { sensors.net[2+bank*8]=down }
                if (sensors.raw[10+bank*16]>thd || sensors.raw[12+bank*16]>thd){ sensors.net[3+bank*8]=down }
                if (sensors.raw[0+bank*16]>thd  || sensors.raw[6+bank*16]>thd) { sensors.net[4+bank*8]=down }
                if (sensors.raw[9+bank*16]>thd  || sensors.raw[15+bank*16]>thd){ sensors.net[5+bank*8]=down }
                if (sensors.raw[1+bank*16]>thd  || sensors.raw[5+bank*16]>thd) { sensors.net[6+bank*8]=down }
                if (sensors.raw[8+bank*16]>thd  || sensors.raw[14+bank*16]>thd){ sensors.net[7+bank*8]=down }
        } 
        for i:=0; i<rows*cols; i++ {
